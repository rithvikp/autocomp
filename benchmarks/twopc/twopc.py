...
class TwoPCSuite(benchmark.Suite[Input, Output]):
    def run_benchmark(self, bench: benchmark.BenchmarkDirectory,
        ...
        endpoints, receive_endpoints = self.provisioner.rebuild(1, {
            ...
            "replicas": ["leaders", "leaders"],
        }, {"clients": inp.num_client_procs, "leaders": 1, "replicas": inp.num_replicas})
        ...
        if self.service_type("leaders") == "scala":
            raise ValueError("Scala leaders are not currently supported")

        bench.log('Leader started.')

        # Launch replicas
        if self.service_type("replicas") == "scala":
            raise ValueError("Scala replicas are not currently supported")
