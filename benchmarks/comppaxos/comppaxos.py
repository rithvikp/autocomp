...
class Input(NamedTuple):
    ...
    num_leaders: int
    num_acceptor_rows: int
    num_acceptor_columns: int
    num_replicas: int
...
class CompPaxosNet:
    ...
    def prom_placement(self) -> Placement:
        ...
        return self.Placement(
            ...
            acceptors=portify('acceptors', self._input.num_acceptor_rows*self._input.num_acceptor_columns),
            replicas=portify('replicas', self._input.num_replicas),
            proxy_leaders=portify('proxy_leaders', self._input.num_proxy_leaders),
        )
        ...
        return {
            ...
            'acceptor_address': [{
                'acceptor_address': [{
                    'host': e.host.ip(),
                    'port': e.port
                } for e in self.placement(index=index).acceptors[i*self._input.num_acceptor_columns:(i+1)*self._input.num_acceptor_columns]]
            } for i in range(self._input.num_acceptor_rows)],
            'replica_address': [{
                'host': e.host.ip(),
                'port': e.port
...
# Suite ########################################################################
class CompPaxosSuite(benchmark.Suite[Input, Output]):
    def __init__(self):
        self.run_index = 0
        super().__init__()
    ...
    def run_benchmark(self,
                      ...
                      args: Dict[Any, Any],
                      input: Input) -> Output:
        net = CompPaxosNet(input, self.provisioner.hosts(input.f), self.run_index)
        ...
        if self.service_type("acceptors") == "hydroflow":
            ...
            for r in range(input.num_acceptor_rows):
                for (c, acceptor) in enumerate(net.prom_placement().acceptors[r*input.num_acceptor_columns:(r+1)*input.num_acceptor_columns]):
                    index = r*input.num_acceptor_rows + c
                    acceptor_procs.append(self.provisioner.popen_hydroflow(bench, f'acceptors_{index}', input.f, [
                        ...
                        '--acceptor.column',
                        str(c),
                        '--acceptor.num-proxy-leaders',
                        str(input.num_proxy_leaders),
                        '--prometheus-host',
                        acceptor.host.ip(),
                        f'--prometheus-port={str(acceptor.port)}',
                    ]))
        else:
            raise ValueError("CompPaxos only supports hydroflow acceptors")
        ...
        for (i, leader) in enumerate(net.prom_placement().leaders):
            leader_procs.append(self.provisioner.popen_hydroflow(bench, f'leaders_{i}', input.f, [
                ...
                '--leader.f',
                str(input.f),
                '--leader.num-acceptor-rows',
                str(input.num_acceptor_rows),
                '--leader.num-acceptor-columns',
                str(input.num_acceptor_columns),
                '--leader.num-proxy-leaders',
                str(input.num_proxy_leaders),
                '--prometheus-host',
                leader.host.ip(),
        ...
        for (i, proxy_leader) in enumerate(net.prom_placement().proxy_leaders):
            proxy_leader_procs.append(self.provisioner.popen_hydroflow(bench, f'proxy_leaders_{i}', input.f, [
                ...
                '--proxy-leader.index',
                str(i),
                '--proxy-leader.num-acceptor-rows',
                str(input.num_acceptor_rows),
                '--proxy-leader.num-acceptor-columns',
                str(input.num_acceptor_columns),
                '--proxy-leader.f',
                str(input.f),
                '--prometheus-host',
        ...
        endpoints, receive_endpoints = self.provisioner.rebuild(1, {
            "clients": ["leaders"],
            "leaders": ["proxy_leaders", "acceptors", "leaders"],
            "proxy_leaders": ["acceptors", "replicas"],
            "acceptors": ["proxy_leaders", "leaders", "leaders", "leaders"],
            "replicas": ["clients"],
        },
        {
        ...
        return CompPaxosOutput(read_output = read_output,
