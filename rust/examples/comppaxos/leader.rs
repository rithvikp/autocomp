...
pub struct LeaderArgs {
    ...
    #[clap(long = "leader.num-acceptor-rows")]
    leader_num_acceptor_rows: Option<u32>,

    #[clap(long = "leader.num-acceptor-columns")]
    leader_num_acceptor_columns: Option<u32>,

    #[clap(long = "leader.num-proxy-leaders")]
    leader_num_proxy_leaders: Option<u32>,
}
...
pub async fn run(cfg: LeaderArgs, mut ports: HashMap<String, ServerOrBound>) {
    ...
    let num_proxy_leaders = cfg.leader_num_proxy_leaders.unwrap();
    let num_acceptor_rows = cfg.leader_num_acceptor_rows.unwrap();
    let num_acceptor_columns = cfg.leader_num_acceptor_columns.unwrap();
...
# p2bU: maxBallotID, maxBallotNum
.async nacks `null::<(u32,u32)>()` `source_stream(nack_source) -> map(|v| deserialize_from_bytes::<(u32,u32)>(v.unwrap().1).unwrap())`
...
p2aOut((slot%n), i, no, slot, i, num) :- FilledHoles(no, slot), id(i), ballot(num), numProxyLeaders(n) # Weird bug where if this line is commented out, id has an error?
...
RelevantP1bs(ar, ac, logSize) :- p1b(ar, ac, logSize, i, num, maxID, maxNum), id(i), ballot(num)
receivedBallots(id, num) :- iAmLeader(id, num)
receivedBallots(maxBallotID, maxBallotNum) :- p1b(ar, ac, logSize, i, num, maxBallotID, maxBallotNum)
receivedBallots(maxBallotID, maxBallotNum) :- nacks(maxBallotID, maxBallotNum)
...
RelevantP1bLogs(ar, ac, payload, slot, payloadBallotID, payloadBallotNum) :- p1bLog(ar, ac, payload, slot, payloadBallotID, payloadBallotNum, i, num), id(i), ballot(num)

# cannot send new p2as until all p1b acceptor logs are PROCESSED; otherwise might miss pre-existing entry
P1bLogFromAcceptor(ar, ac, count(slot)) :- RelevantP1bLogs(ar, ac, payload, slot, payloadBallotID, payloadBallotNum)
P1bAcceptorLogReceived(ar, ac) :- P1bLogFromAcceptor(ar, ac, logSize), RelevantP1bs(ar, ac, logSize)
P1bAcceptorLogReceived(ar, ac) :- RelevantP1bs(ar, ac, logSize), (logSize == 0)
# Read quorum = have we seen an entire row.
P1bNumAcceptorPartitionsLogReceived(count(ac), ar) :- P1bAcceptorLogReceived(ar, ac)
P1bQuorum() :- P1bNumAcceptorPartitionsLogReceived(n, acceptorID), numAcceptorColumns(n)
IsLeader() :- P1bQuorum(), HasLargestBallot()

# what was not committed = find max ballot, store in local log, resend 
P1bLargestEntryBallotNum(slot, max(payloadBallotNum)) :- RelevantP1bLogs(ar, ac, payload, slot, payloadBallotID, payloadBallotNum)
P1bLargestEntryBallot(slot, max(payloadBallotID), payloadBallotNum) :- P1bLargestEntryBallotNum(slot, payloadBallotNum), RelevantP1bLogs(ar, ac, payload, slot, payloadBallotID, payloadBallotNum)
...
ResentLog(payload, slot) :- !nextSlot(s), IsLeader(), P1bLargestEntryBallot(slot, payloadBallotID, payloadBallotNum), RelevantP1bLogs(ar, ac, payload, slot, payloadBallotID, payloadBallotNum)
...
p2a@(slot%n)(i, payload, (slot + offset), i, num) :~ IndexedPayloads(payload, offset), nextSlot(slot), id(i), ballot(num), numProxyLeaders(n)
