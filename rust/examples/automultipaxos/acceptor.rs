...
pub async fn run(cfg: AcceptorArgs, mut ports: HashMap<String, ServerOrBound>) {
    ...
    let acceptor_id = cfg.acceptor_index.unwrap();
    let partition_id = cfg.acceptor_partition_index.unwrap();
    let coordinator = cfg.acceptor_coordinator_index.unwrap();
    let num_p2b_proxy_leaders = cfg.acceptor_num_p2b_proxy_leaders.unwrap();
...
.output p1bLogOut `for_each(|(pid,p,a,payload,slot,payload_id,payload_num,id,num):(u32,u32,u32,Rc<Vec<u8>>,u32,u32,u32,u32,u32,)| println!("acceptor {:?} sent p1bLog to proposer {:?}: [{:?},{:?},{:?},{:?},{:?},{:?},{:?},{:?}]", p, pid, p, a, payload, slot, payload_id, payload_num, id, num))`
.output p2aOut `for_each(|(p,pid,payload,slot,id,num):(u32,u32,Rc<Vec<u8>>,u32,u32,u32,)| println!("acceptor {:?} received p2a from p2aProxyLeader: [{:?},{:?},{:?},{:?},{:?}]", p, pid, payload, slot, id, num))`
...
.output p2bOut `for_each(|(p,pid,a,payload,slot,id,num,max_id,max_num):(u32,u32,u32,Rc<Vec<u8>>,u32,u32,u32,u32,u32,)| println!("acceptor {:?} sent p2b to {:?} p2bProxyLeader: [{:?},{:?},{:?},{:?},{:?},{:?},{:?}]]", p, pid, a, payload, slot, id, num, max_id, max_num))`
# For some reason Hydroflow can't infer the type of p2aSealed, so we define it manually:
.input p2aSealed `null::<(u32,Rc<Vec<u8>>,u32,u32,u32)>()`

# p1a: proposerID, ballotID, ballotNum
.async p1aU `null::<(u32,u32,u32,)>()` `source_stream(p1a_source) -> map(|v| deserialize_from_bytes::<(u32,u32,u32,)>(v.unwrap().1).unwrap())`
...
.async p2aU `null::<(u32,Rc<Vec<u8>>,u32,u32,u32,)>()` `source_stream(p2a_source) -> map(|v| deserialize_from_bytes::<(u32,Rc<Vec<u8>>,u32,u32,u32,)>(v.unwrap().1).unwrap())`
...
.async p1aCommitU `null::<(u32,u32,u32,u32,)>()` `source_stream(p1a_commit_source) -> map(|v| deserialize_from_bytes::<(u32,u32,u32,u32,)>(v.unwrap().1).unwrap())`
