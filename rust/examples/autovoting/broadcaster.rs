...
.async toBroadcaster `null::<(u32,i64,)>()` `source_stream(to_broadcaster_source) -> map(|x| deserialize_from_bytes::<(u32,i64,)>(x.unwrap().1).unwrap())`
.async voteToParticipants `map(|(node_id, v)| (u32::try_from(node_id).unwrap(), serialize_to_bytes(v))) -> dest_sink(to_participant_sink)` `null::<(u32,i64,)>()`  

voteToParticipants@(p+(v%m))(client, v) :~ toBroadcaster(client, v), participantStartIDs(p), numParticipantPartitions(m)
    "#
    );
