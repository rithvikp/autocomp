...
collisions(v2, hashed) :- voteToReplica(client, id, b, v1, hashed), storage(v2, hashed)
voteFromReplica@addr(client, id, b, v2) :~ voteToReplica(client, id, b, v, hashed), collisions(v2, hashed), leader(addr), myID(i)
voteFromReplica@addr(client, id, b, v) :~ voteToReplica(client, id, b, v, hashed), !collisions(_, hashed), leader(addr), myID(i)
        "#
    );
